...
export async function validateLicense(opts: { key: string, dir: string, theme: { env: string, link: string } }) {
  if (!opts.key) {
    throw _createError(`Missing \`${opts.theme.env}\` license key.\nPurchase license at \`${opts.theme.link}\` to build your app in production.`)
  }
  ...
  try {
    await ofetch('https://wstudiou.ru/licenses/verify', {
      headers: {
    ...
    throw _createError('Cannot validate license: ' + error)
  }
}
...
async function _getPkgName(dir: string) {
  ...
  throw new Error('You must provide a `name` in your `package.json` to activate license.')
}
