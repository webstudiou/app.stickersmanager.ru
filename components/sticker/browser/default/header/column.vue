...
const columnStyle = (column: StickerBrowserDefaultColumn) => {
  return {
...
<template>
  <div v-if="column.visible" class="relative whitespace-nowrap group" :style="columnStyle(column)">
    <div class="flex items-center h-full -ml-[8px] pl-[8px] transition-[background-color] rounded-[4px]">
      <div class="truncate">
        {{ column.key }}
      </div>
      <div v-if="column.resize" class="absolute cursor-grab -right-[5px] w-[10px] px-[2px] transition-[opacity] opacity-0 group-hover:opacity-100">
